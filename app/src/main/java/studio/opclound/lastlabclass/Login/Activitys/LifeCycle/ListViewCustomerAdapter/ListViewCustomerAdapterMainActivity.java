...
public class ListViewCustomerAdapterMainActivity extends AppCompatActivity {
    /*
        1 - Create a specific design for the item list: it may contain two or more view to be populated
     */
    //The arrayList to populate the listView
    private ArrayList<ListViewCustomerAdapterStudent> studentsList = new ArrayList<ListViewCustomerAdapterStudent>();

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.list_view_customer_adapter_activity_main);
         ...
         */
        ListViewCustomerAdapterStudentArrayAdapter listViewCustomerAdapterStudentArrayAdapter =
                new ListViewCustomerAdapterStudentArrayAdapter(
                        getBaseContext(), // Current context
                        R.layout.list_view_customer_adapter_item_layout, // the layout for each item in the list
                        studentsList); // the arrayList to feed the arrayAdapter

        //5 - The last step is to connect the listView and the Java class and to populate the list View using the ArrayAdapter
        ListView listView = findViewById(R.id.listView);
        listView.setAdapter(listViewCustomerAdapterStudentArrayAdapter);
    ...
    private void generateDummyContent() {
        ...
        for (String element : nameDummy) {
            ...
            studentsList.add(new ListViewCustomerAdapterStudent(studentName, Integer.valueOf(studentId)));
